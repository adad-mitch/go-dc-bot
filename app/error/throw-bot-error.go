...
func ThrowBotError(bot *discordgo.Session, channel string, e error) {
	...
	var footer string
	if err != nil {
		footer = "(Sorry, I failed to capture the error log.)"
		log.Println(err)
	} else {
		footer = fmt.Sprintf("CloudWatch Correlation ID: %v", *uuid)
	}
	_, err = bot.ChannelMessageSendEmbed(channel, &discordgo.MessageEmbed{
		Title:       "Uh-oh. Something went wrong.",
		Description: "Search the CloudWatch correlation ID below.",
		Color:       0xDD4400,
